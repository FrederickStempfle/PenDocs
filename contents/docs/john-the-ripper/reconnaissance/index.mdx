...
<Tabs defaultValue="company" className="pt-5 pb-1">
  ...
  <TabsContent value="company">
    ...
    - Company name and variations (CompanyName, company_name)
  ...
  <TabsContent value="industry">
    ...
    - Regulatory frameworks (HIPAA, PCI, etc.)
...
<Steps>
  ...
  <Step title="Optimize for size and efficiency">
    ...
    cat processed_wordlist.txt | awk '{print length, $0}' | sort -n | cut -d " " -f2- > optimized_wordlist.txt
  ...
  <Step title="Test and refine">
    ...
    ```bash
    # Test against sample hashes (if available)
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
  <Card>
    ...
    <CardContent>
      ...
      ```bash
      # Network scanning (when authorized)
...
<Tabs defaultValue="windows" className="pt-5 pb-1">
  ...
  <TabsContent value="windows">
    ...
    ```bash
    # Using enum4linux (when authorized)
    enum4linux -a 10.0.0.1
    
    # Using net commands (requires access)
    net user /domain
    net group "Domain Admins" /domain
    
    # Using PowerShell (requires access)
  ...
  <TabsContent value="linux">
    ...
    ```bash
    # Local user enumeration (requires access)
    cat /etc/passwd
    
    # LDAP enumeration (when authorized)
    ldapsearch -x -h 10.0.0.1 -b "dc=example,dc=com" "(objectClass=user)"
...
<Callout type="info">
  Threat modeling helps identify and prioritize potential password-related vulnerabilities. This process ensures that your password cracking efforts are focused on the most critical areas.
</Callout>
...
<Tabs defaultValue="complexity" className="pt-5 pb-1">
  ...
  <TabsContent value="complexity">
    ...
    ```powershell
    # Windows domain policy (requires access)
  ...
  <TabsContent value="history">
    ...
    - Seasonal variations (Company_Spring2023)
    - Character substitutions (p@ssword, p@$$word)
...
<Note title="Ethical Reminder" type="warning">
  All intelligence gathering activities must be conducted within the authorized scope of the penetration test. Unauthorized reconnaissance can violate privacy laws and terms of service agreements. Always ensure you have proper authorization before collecting information about an organization or its employees.
</Note>
...
3. Prepare for [password cracking](/docs/john-the-ripper/post-exploitation) using the intelligence you've gathered

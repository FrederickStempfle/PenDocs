...
<Tabs defaultValue="enumeration" className="pt-5 pb-1">
  ...
  <TabsContent value="enumeration">
    ...
    Always start with enumeration to understand the database structure before attempting to extract specific data.
    </Note>
  </TabsContent>
  <TabsContent value="tables">
    ...
    <Note title="Security Note" type="warning">
    Tables with names like "users", "accounts", "members", or "admin" often contain valuable information such as usernames, email addresses, and password hashes.
    </Note>
  </TabsContent>
  <TabsContent value="dump">
    ...
    Dumping entire databases can generate a large amount of traffic and may trigger intrusion detection systems. Use this option with caution.
    </Note>
  </TabsContent>
</Tabs>
...
<Note title="Ethical Reminder" type="note">
Remember that unauthorized data extraction is illegal. Always obtain proper authorization before performing security testing on any system.
</Note>
...
<Note title="Performance Tip" type="success">
When extracting large amounts of data, use the `--threads` parameter to speed up the process:

```bash
sqlmap -u "http://target.com/vuln.php?id=1" -D database_name -T large_table --dump --threads=10
```

This runs multiple threads in parallel, significantly reducing extraction time.
</Note>
